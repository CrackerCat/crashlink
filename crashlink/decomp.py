...
class IRFunction:
    ...
    def _lift_block(self, node: CFNode, visited: Optional[Set[CFNode]] = None) -> IRBlock:
        ...
        for i, op in enumerate(node.ops):
            if op.op == "Label": # HACK: loop handling should be done with cfg, not with labels. but this works, so whatever.
                assert i == 0, "Label should be the first operation in a CFNode unconditionally. If this breaks, check CFG generation."
